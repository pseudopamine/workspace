...
# Flask 백엔드에 LED 상태 확인 엔드포인트 추가
@app.route('/api/control/led/status', methods=['GET'])
def get_led_status():
    """현재 LED 상태 반환"""
    try:
        # LED_PIN의 상태 읽기
        pin_state = GPIO.input(LED_PIN)
        state = 'on' if pin_state == GPIO.HIGH else 'off'
        return jsonify({
            "success": True, 
            "state": state
        })
...
def control_led():
    ...
    try:
        if state == 'on':
            GPIO.output(LED_PIN, GPIO.HIGH)
            # 실제 핀 상태 확인
            actual_state = GPIO.input(LED_PIN)
            if actual_state != GPIO.HIGH:
                return jsonify({
                    "success": False, 
                    "error": "LED 상태 변경 실패", 
                    "requested": "on", 
            ...
            return jsonify({
                "success": False, 
                "error": "유효하지 않은 상태입니다. 'on' 또는 'off'만 가능합니다."
            }), 400
