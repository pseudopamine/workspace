...
class AutoController:
    def __init__(self, db_config, gpio_setup):
        ...
        self.running = False
        self.thread = None
        self.control_interval = 300  # 5분(300초) 간격으로 제어
    ...
    def control_no2(self, no2, settings):
        """이산화질소 제어 로직"""
        if no2 >= settings['DAN_NO2']:
            # 위험 수준 - 강제 환기 작동 (서보모터 180도)
            self.gpio.control_servo(180)
            return f"NO2 농도 {no2}가 위험값({settings['DAN_NO2']}) 이상: 강제 환기 작동"
        elif no2 >= settings['BOU_NO2']:
            # 경계 수준 - 일반 환기 작동 (서보모터 120도)
            self.gpio.control_servo(120)
            return f"NO2 농도 {no2}가 경계값({settings['BOU_NO2']}) 이상: 일반 환기 작동"
        return None
    ...
    def control_co2(self, co2, settings):
        ...
        if co2 >= settings['DAN_CO2']:
            ...
            time.sleep(0.2)
            self.gpio.output(self.gpio.LED_PIN, self.gpio.LOW)
            time.sleep(0.2)
            self.gpio.output(self.gpio.LED_PIN, self.gpio.HIGH)
            time.sleep(0.2)
            self.gpio.output(self.gpio.LED_PIN, self.gpio.LOW)
            return f"CO2 농도 {co2}가 위험값({settings['DAN_CO2']}) 이상: 경보 작동"
        elif co2 >= settings['BOU_CO2']:
            # 경계 수준 - LED 한번 깜빡임
            self.gpio.output(self.gpio.LED_PIN, self.gpio.HIGH)
            time.sleep(0.5)
            self.gpio.output(self.gpio.LED_PIN, self.gpio.LOW)
            return f"CO2 농도 {co2}가 경계값({settings['BOU_CO2']}) 이상: 주의 신호"
        return None
    ...
    def control_toluene(self, toluene, settings):
        """톨루엔 제어 로직"""
        if toluene >= settings['DAN_TOLUENE']:
            # 위험 수준
            return f"톨루엔 농도 {toluene}가 위험값({settings['DAN_TOLUENE']}) 이상: 알림 필요"
        elif toluene >= settings['BOU_TOLUENE']:
            # 경계 수준
            return f"톨루엔 농도 {toluene}가 경계값({settings['BOU_TOLUENE']}) 이상: 주의 필요"
        return None
